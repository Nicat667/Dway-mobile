...
// Scheduled when the timer starts so the OS fires the alarm even if the app
// is backgrounded or killed before the countdown ends.
let _timerNotificationId: string | null = null;
...
export async function scheduleTimerNotification(
  ...
  try {
    ...
    _timerNotificationId = id;
  } catch {}
}

export async function cancelTimerNotification(): Promise<void> {
  try {
    if (_timerNotificationId) {
      await Notifications.cancelScheduledNotificationAsync(_timerNotificationId);
      _timerNotificationId = null;
    }
  } catch {}
}
