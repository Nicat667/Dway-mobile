...
import { cancelTimerNotification, scheduleTimerNotification } from "@/utils/alarmService";
...
export default function TimerModal({ visible, onClose }: Props) {
  ...
  const { profileSettings, triggerTimerAlarm } = useApp();

  // Stop timer tracking when modal is closed externally
  useEffect(() => {
    if (!visible) {
      setIsRunning(false);
    }
  ...
  // Tracks when the timer should end (epoch ms) for background recalculation
  ...
  useEffect(() => {
    const handleAppState = (nextState: AppStateStatus) => {
      if (nextState === "active" && endTimeRef.current !== null) {
        ...
        if (left === 0) {
          // Timer finished while in background — cancel the OS notification
          // (it already fired) and show the fullscreen overlay
  ...
  const toggleTimer = () => {
    ...
    if (!started || remaining === 0) {
      ...
      const endAt = Date.now() + t * 1000;
      endTimeRef.current = endAt;
      setTotalSeconds(t);
      setRemaining(t);
      setStarted(true);
      setIsRunning(true);
      // Schedule OS notification so alarm fires even if app is backgrounded
      scheduleTimerNotification(new Date(endAt), profileSettings.alarmSound);
    } else if (isRunning) {
      // Pause — cancel the scheduled notification; we'll reschedule on resume
      cancelTimerNotification();
      endTimeRef.current = null;
      setIsRunning(false);
    } else {
      // Resume — reschedule based on current remaining
