...
function getAIResponse(userMsg: string, taskCount: number, completedCount: number): string {
  ...
  return `I'm DWAY AI — here to support and guide you. You have ${taskCount} tasks, having completed ${completedCount} (${completionRate}%). Let's work together to improve. What specific area would you like help with — time management, focus, or setting better goals?`;
}

export default function DwayAiModal({ visible, onClose }: Props) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hello! I'm DWAY AI. I'm here to help you stay focused, motivated, and on track with your goals. How can I support you today?",
    },
  ]);
  ...
  const sendMessage = (text: string) => {
    ...
    setTimeout(() => {
      const aiMsg: Message = {
        ...
        role: "assistant",
        content: getAIResponse(text, tasks.length, completedTasks),
      };
  ...
  const styles = StyleSheet.create({
    ...
    container: {
      ...
      marginTop: insets.top + 20, borderTopLeftRadius: 28, borderTopRightRadius: 28,
    },
    header: {
    ...
    headerSub: { fontSize: 12, color: colors.success, fontFamily: "Inter_400Regular", marginTop: 1 },
    closeBtn: { padding: 6 },
    messages: { flex: 1, padding: 16 },
    messageBubble: { maxWidth: "80%", marginBottom: 12, borderRadius: 18, padding: 14 },
    ...
    typingIndicator: {
      ...
      borderRadius: 18, borderBottomLeftRadius: 4, padding: 14, marginBottom: 12,
    },
    ...
    suggestionsLabel: {
      fontSize: 11, color: colors.mutedForeground, fontFamily: "Inter_500Medium",
      marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5,
    },
    suggestionChips: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
    suggestionChip: { backgroundColor: colors.muted, borderRadius: 16, paddingHorizontal: 12, paddingVertical: 6 },
    suggestionText: { fontSize: 12, color: colors.foreground, fontFamily: "Inter_400Regular" },
    inputRow: {
      flexDirection: "row", alignItems: "center",
    ...
    textInput: {
      flex: 1, backgroundColor: colors.muted, borderRadius: 22,
            ...
            <View style={styles.container}>
              <View style={styles.header}>
                ...
                <View style={styles.headerInfo}>
                  <Text style={styles.headerTitle}>DWAY AI</Text>
              ...
              >
                {messages.map((msg) => (
                  <View key={msg.id} style={[styles.messageBubble, msg.role === "user" ? styles.userBubble : styles.aiBubble]}>
                    <Text style={[styles.messageText, { color: msg.role === "user" ? "#fff" : colors.foreground }]}>
                      {msg.content}
                    </Text>
                  </View>
                ))}
